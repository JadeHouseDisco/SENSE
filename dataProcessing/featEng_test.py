...
class SEMGDataGenerator(Sequence):
    ...
    def __getitem__(self, index):
        ...
        return [X, gender_features], y  # Return both sEMG data and gender feature as inputs
    ...
    def __data_generation(self, batch_segments):
        ...
        with h5py.File(self.hdf5_file, 'r') as f:
            for segment_name in batch_segments:
                ...
                # Assign label based on the segment name (0 for Young, 1 for Old)
...
# Prepare test data based on subject ID
...
for (X_batch, gender_batch), y_batch in test_data_generator:
    preds = model.predict([X_batch, gender_batch])
    y_pred.extend(preds)
