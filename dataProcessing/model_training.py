...
class SEMGDataGenerator(Sequence):
    def __init__(self, hdf5_file, segment_names, batch_size=4, shuffle=True):
        ...
        self.segments = segment_names  # Pass the segments for training or testing
...
if __name__ == "__main__":
    ...
    number_of_muscles, frequency_bins, time_points = extract_data_dimensions(hdf5_file) # 10000, 47, 13
    ...
    model = tf.keras.models.Sequential([
        tf.keras.layers.Conv2D(32, (3, 3), activation='relu', input_shape=(frequency_bins, time_points, number_of_muscles)),
        tf.keras.layers.BatchNormalization(),
        tf.keras.layers.MaxPooling2D((2, 2)),

        tf.keras.layers.Conv2D(64, (3, 3), activation='relu'),
        tf.keras.layers.BatchNormalization(),
        # tf.keras.layers.MaxPooling2D((2, 2)), Removed to avoid over-downsampling
